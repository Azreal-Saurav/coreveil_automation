...
export default function ContactSection() {
  ...
  const handleSubmit = () => {
    ...
    setStatus('sending');
    const message = `New Lead from Coreveil Automation Website!

Name: ${formData.name}
Email: ${formData.email}
Business: ${formData.business || 'N/A'}
Services Interested: ${formData.services.join(', ') || 'None selected'}
Message: ${formData.message || 'N/A'}`.trim();

    setTimeout(() => {
        const emailSubject = encodeURIComponent('New Lead - Coreveil Automations');
        const emailBody = encodeURIComponent(message);
        window.location.href = `mailto:aseemmadaan9@gmail.com?subject=${emailSubject}&body=${emailBody}`;
